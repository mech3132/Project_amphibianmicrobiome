...
for ( beta in as.vector(allBeta$V1) ) {
    ...
    for ( sp in levels(factor(mf_alt$spec))) {
        ...
        disp.temp <- vegan::betadisper(as.dist(current.dm), group = (current.mf$time_point_ranked), type = "centroid")
...
BD <- mf_alt %>%
  as_tibble() %>%
  dplyr::select(SampleID, bd_run_1,bd_run_2, bd_average_run_3) %>% rowwise() %>%
  mutate(Keep = ifelse(sum(c(bd_run_1,bd_run_2,bd_average_run_3), na.rm = TRUE) > 0, TRUE, FALSE)) %>%
  ungroup() %>% filter(Keep) %>% dplyr::select(-Keep, -SampleID)
new_sampIDs <- mf_alt %>%
  as_tibble() %>%
  dplyr::select(SampleID, bd_run_1,bd_run_2, bd_average_run_3) %>% rowwise() %>%
  mutate(Keep = ifelse(sum(c(bd_run_1,bd_run_2,bd_average_run_3), na.rm = TRUE) > 0, TRUE, FALSE)) %>%
  ungroup() %>% filter(Keep) %>% dplyr::select(SampleID)
...
BD_lambda_est <- t(apply(round(log(BD+1)), MARGIN=1, FUN=function(x) {
    temp <- fitdistr(x[!is.na(x)], densfun="Poisson")
...
final_BD <- cbind(new_sampIDs, BD,BD_lambda_est)

BD_lambda_est_old <- t(apply(round(log(BD_old+1)), MARGIN=1, FUN=function(x) {
    temp <- fitdistr(x[!is.na(x)], densfun="Poisson")
    return(c(lambda=temp$estimate,sd=temp$sd))
})) %>%
    as_tibble() %>%
    ...
    mutate(pval=dnorm(0,mean=lambda,sd=sd)) %>% # To estimate if lambda is significantly different from zero, we see if pval for parameter estimate different from zero
    mutate(sig=pval<=0.10)
final_BD_old <- cbind(old_sampIDs,BD_old,BD_lambda_est_old)
...
for ( r in 1:nrow(BD_alt) ) {
    ...
    if ( (sum(BD_alt[r,] == 0, na.rm=TRUE) == 2) & (max(BD_alt[r,],na.rm=TRUE) < thirdTHRESH) ) {
        BD_alt[r,] <- data.frame(bd_run_1 = 0,bd_run_2 = 0,bd_average_run_3 = 0)
    }
    
}
# BD_alt$infected <- apply(BD_alt,1, function(x) mean(x,na.rm = TRUE)) >5
BD_alt <- BD_alt %>%  rowwise() %>% mutate(logestimate = mean(c(log(bd_run_1+1),log(bd_run_2+1), log(bd_average_run_3+1)), na.rm=TRUE), nest = sum(c(!is.na(bd_run_1), !is.na(bd_run_2), !is.na(bd_average_run_3))), sdestimate = sd(c(log(bd_run_1+1),log(bd_run_2+1),log(bd_average_run_3+1)), na.rm=TRUE), infected = logestimate>1 )
cbind(final_BD[,c("bd_run_1","bd_run_2","bd_average_run_3","lambda","sig")], alt_est = BD_alt$logestimate, alt_infect =BD_alt$infected) %>% ggplot() + geom_point(aes(x=lambda, y=alt_est, col=sig))
...
allBdStats <- cbind(new_sampIDs, logestimate = BD_alt$logestimate, sdestimate = BD_alt$sdestimate) 
# allBdStats <- final_BD %>% filter(sig)

mf_alt$eBD <- 0
mf_alt$sdBD <- 0
mf_alt$eBD2 <- 0
mf_alt$sdBD2 <- 0
mf_alt[match(final_BD$SampleID, mf_alt$SampleID),"eBD2"] <- final_BD$lambda
mf_alt[match(final_BD$SampleID, mf_alt$SampleID),"sdBD2"] <- final_BD$sd
mf_alt[match(allBdStats$SampleID, mf_alt$SampleID),"eBD"] <- allBdStats$logestimate
mf_alt[match(allBdStats$SampleID, mf_alt$SampleID),"sdBD"] <- allBdStats$sdestimate
...
mf_alt_final <- mf_alt %>% # mapping file with controls still in it
    ...
    rename_at(vars(toKeepMF), ~ all_of(newNames)) %>%
...
mf_alt_filt_final <- mf_alt_filt %>% # mapping file without controls
    as_tibble() %>% # make into tibble
    dplyr::select(one_of(toKeepMF)) %>% # filter to only relevant variables
    rename_at(vars(toKeepMF), ~ all_of(newNames)) %>% #rename variable names
